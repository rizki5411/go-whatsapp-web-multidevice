...
| Add DB migration | `src/infrastructure/chatstorage/sqlite_repository.go` `getMigrations()` | Append only, one statement per entry. Current list has 62 migrations. |
| Multi-tenant / device ownership | `src/domains/tenancy/`, `src/ui/rest/middleware/authgate.go`, `device_owner_guard.go`, `principal.go`, `src/ui/rest/tenantfilter/`, `src/usecase/tenancy.go`, `device_ownership.go` | Behind `MULTI_TENANT_ENABLED` (default off). Routes taking `:device_id` in the path MUST call `tenantfilter.GuardParamDevice`; header/query routes are covered by `DeviceOwnerGuard`. Ops guide: `docs/multitenant/OPERASI.md`. |
